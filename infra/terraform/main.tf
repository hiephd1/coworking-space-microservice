...
resource "aws_subnet" "project3-pub-subnet" {
  vpc_id                  = aws_vpc.vpc.id
  cidr_block              = "10.0.10.0/24"
  availability_zone       = "us-east-1${var.public_az}"
  map_public_ip_on_launch = true
  tags = {
    Name = "project3-public"
  }
}
...
resource "aws_route_table_association" "public-route-table" {
  subnet_id      = aws_subnet.project3-pub-subnet.id
  route_table_id = aws_route_table.public-route-table.id
}

# Create a private subnet
resource "aws_subnet" "project3-pri-subnet" {
  vpc_id            = aws_vpc.vpc.id
  availability_zone = "us-east-1${var.private_az}"
  cidr_block        = "10.0.20.0/24"
  tags = {
    Name = "project3-private"
  }
}
...
resource "aws_route_table_association" "private-route-table" {
  subnet_id      = aws_subnet.project3-pri-subnet.id
...
resource "aws_vpc_endpoint" "eks" {
  ...
  subnet_ids          = [aws_subnet.project3-pri-subnet.id]
...
resource "aws_eks_cluster" "main" {
  ...
  vpc_config {
    subnet_ids              = [aws_subnet.project3-pri-subnet.id, aws_subnet.project3-pub-subnet.id]
...
resource "aws_eks_node_group" "main" {
  ...
  subnet_ids      = [var.enable_private == true ? aws_subnet.project3-pri-subnet.id : aws_subnet.project3-pub-subnet.id]
